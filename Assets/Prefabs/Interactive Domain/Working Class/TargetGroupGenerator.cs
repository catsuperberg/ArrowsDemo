    ...
    namespace Track
    {
        public class TargetGroupGenerator : MonoBehaviour, ITargerProvider 
        {
            public GameObject GetSuitableTarget(List<GameObject> targetPrefabs, BigInteger targetResult, (int Min, int Max) numberOfTargetsRange)
            {
                ...
                var targetGroup = Instantiate(gameObject, UnityEngine.Vector3.zero, UnityEngine.Quaternion.identity);
                ...
                foreach(BigInteger score in targetScores)
                {
                    ...
                    targets.Add(CreateTargetFromPrefab(randomPrefab, score));
                }
                var targetsArea = GameObjectUtils.GetAreaOfObjectsWithColliders(targets)*1.5;
                var radius = (float)System.Math.Sqrt(targetsArea/System.Math.PI);
                PlaceInsideCircle(targets, radius);
                
                foreach(GameObject entry in targets)
