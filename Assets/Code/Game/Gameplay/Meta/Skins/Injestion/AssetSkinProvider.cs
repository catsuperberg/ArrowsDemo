...
namespace Game.Gameplay.Meta.Skins
{
    public class AssetSkinProvider<T> : ISkinProvider where T : ISkinData<T>, ISkinData
    {
        ...
        {
            var skinData = GetSkinIfValid(name);
            return Resources.Load<Sprite>(skinData.IconPath.GetAtResourcesWithNoExtension()) ?? 
                throw new MissingFieldException($"Couldn't load icon sprite from {skinData.IconPath}");
        }    
